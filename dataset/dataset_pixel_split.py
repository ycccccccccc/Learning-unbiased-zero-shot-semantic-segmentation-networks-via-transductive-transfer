...
from dataset.transform_pixel_split import *
...
def transform_for_test(fixed_scale=512, classes=None,split = None):
    transform_list = []
...
def prepare_for_train_pixel_dataloader(dataroot, bs_train=4, input_size=(512, 512), shuffle=True, num_workers=2,split = None):
    """
    use pascal 2012, dataroot contain JEPG/SEGMENTATION/so on
    """
    name = "split" + split + "_"+ "train_strong"
    with open(os.path.join(dataroot,name +".txt"), "r") as f:
    ...
    transform = transform_for_train(fixed_scale=input_size, rotate_prob=15, classes=classes,split = split)

    voc_train = VOCSegmentation(base_dir=dataroot, split=name, transform=transform)
...
def prepare_for_train_weak_pixel_dataloader(dataroot, bs_train=4, input_size=(512, 512), shuffle=True, num_workers=2,split = None):
    """
    use pascal 2012, dataroot contain JEPG/SEGMENTATION/so on
    """
    name = "split" + split + "_"+ "train_weak"
    with open(os.path.join(dataroot, name +".txt"), "r") as f:
        lines = f.read().splitlines()
        classes = lines[0]
        lines.remove(classes)
...
def prepare_for_val_pixel_dataloader(dataroot, bs_val=6, input_size=512, shuffle=False, num_workers=2,split = None):
    """
    use pascal 2012
    """
    name = "val"
    ...
    transform = transform_for_test(fixed_scale=input_size,classes = classes,split = split)

    voc_test = VOCSegmentation(base_dir=dataroot, split=name, transform=transform)
...
def prepare_for_val_weak_pixel_dataloader(dataroot, bs_val=6, input_size=512, shuffle=False, num_workers=2,split = None):
    """
    use pascal 2012
    """
    name = "split" + split + "_" +"val_weak"
    with open(os.path.join(dataroot,name  +".txt"), "r") as f:
        lines = f.read().splitlines()
        classes = lines[0]
        lines.remove(classes)
