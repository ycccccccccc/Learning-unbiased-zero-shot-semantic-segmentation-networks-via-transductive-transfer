...
PRETRAINED_OUR_PATH = PATH + PRETRAINED_OUR_PATH + "/" + EMBEDDING
...
def get_pseudo(pre, mask):
    pre = pre[:, 15:, :, :]
    mc = torch.zeros(0).cuda()

    for j in range(pre.shape[0]):
        mp, _ = torch.max(pre[j], 0)
        mp = mp[mask[j] != 255]
        mp = torch.reshape(mp, [-1])
        mc = torch.cat([mc, mp], -1)

    tmp, _ = torch.sort(mc, descending=True)
    index = int(st_lambda * mc.shape[0])
    p = tmp[index]
